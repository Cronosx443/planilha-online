document.addEventListener('DOMContentLoaded', () => {
    // Carrega os dados do usuário e os eventos assim que a página estiver pronta
    loadUserData();
    loadEvents(); 
    ...
    // Configura o salvamento automático dos dados do usuário
    const userFields = ['userName', 'userCpf', 'userPix', 'userBank', 'userAgency', 'userAccount'];
    userFields.forEach(fieldId => {
        const input = document.getElementById(fieldId);
        if (input) {
            input.addEventListener('blur', saveUserData); // 'blur' é acionado quando você clica fora do campo
        }
    });
});

// --- FUNÇÕES PARA SALVAR E CARREGAR SEUS DADOS ---
...
function saveUserData() {
    ...
    // Converte o objeto para texto e salva no localStorage
...
function loadUserData() {
    ...
    if (savedData) {
        const userData = JSON.parse(savedData); // Converte o texto de volta para objeto
...
function renderTable(events) {
    ...
    // Ordena os eventos por data antes de renderizar
...
async function addEvent() {
    ...
    const novoEvento = {
        eventDate: new Date(eventDate + 'T00:00:00'), eventName,
...
function deleteRow(btn) {
    if (confirm('Tem certeza que deseja apagar este evento?')) {
        // No futuro, aqui entraria a lógica para apagar do banco de dados
